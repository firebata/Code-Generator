...
*/
@Controller
@RequestMapping("/sysAppsflyerLog")
public class SysAppsflyerLogController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(SysAppsflyerLogController.class);
	
	private static final String BASE_PATH = "sysAppsflyerLog/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = sysAppsflyerLogService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			SysAppsflyerLog entity = sysAppsflyerLogService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<SysAppsflyerLog>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<SysAppsflyerLog>> result = new HttpResult<Pagination<SysAppsflyerLog>>();
		try {
			Pagination<SysAppsflyerLog> data = sysAppsflyerLogService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
