...
*/
@Controller
@RequestMapping("/appsflyerInformation")
public class AppsflyerInformationController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(AppsflyerInformationController.class);
	
	private static final String BASE_PATH = "appsflyerInformation/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = appsflyerInformationService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			AppsflyerInformation entity = appsflyerInformationService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<AppsflyerInformation>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<AppsflyerInformation>> result = new HttpResult<Pagination<AppsflyerInformation>>();
		try {
			Pagination<AppsflyerInformation> data = appsflyerInformationService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
