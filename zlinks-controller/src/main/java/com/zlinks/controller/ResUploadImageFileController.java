...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.ResUploadImageFile;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.ResUploadImageFileService;
...
*/
@Controller
@RequestMapping("/resUploadImageFile")
public class ResUploadImageFileController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(ResUploadImageFileController.class);
	
	private static final String BASE_PATH = "resUploadImageFile/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = resUploadImageFileService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			ResUploadImageFile entity = resUploadImageFileService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<ResUploadImageFile>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<ResUploadImageFile>> result = new HttpResult<Pagination<ResUploadImageFile>>();
		try {
			Pagination<ResUploadImageFile> data = resUploadImageFileService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
