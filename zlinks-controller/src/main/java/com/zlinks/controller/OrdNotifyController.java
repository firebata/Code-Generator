...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.OrdNotify;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.OrdNotifyService;
...
*/
@Controller
@RequestMapping("/ordNotify")
public class OrdNotifyController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(OrdNotifyController.class);
	
	private static final String BASE_PATH = "ordNotify/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = ordNotifyService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			OrdNotify entity = ordNotifyService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<OrdNotify>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<OrdNotify>> result = new HttpResult<Pagination<OrdNotify>>();
		try {
			Pagination<OrdNotify> data = ordNotifyService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
