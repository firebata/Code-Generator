...
*/
@Controller
@RequestMapping("/usrAccount")
public class UsrAccountController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(UsrAccountController.class);
	
	private static final String BASE_PATH = "usrAccount/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = usrAccountService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			UsrAccount entity = usrAccountService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<UsrAccount>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<UsrAccount>> result = new HttpResult<Pagination<UsrAccount>>();
		try {
			Pagination<UsrAccount> data = usrAccountService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
