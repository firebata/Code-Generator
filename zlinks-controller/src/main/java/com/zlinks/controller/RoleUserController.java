...
public class RoleUserController extends BaseController {
	...
	private static final String BASE_PATH = "roleUser/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = roleUserService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			RoleUser entity = roleUserService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<RoleUser>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<RoleUser>> result = new HttpResult<Pagination<RoleUser>>();
		try {
			Pagination<RoleUser> data = roleUserService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
