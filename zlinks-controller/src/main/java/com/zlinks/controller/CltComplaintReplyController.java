...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.CltComplaintReply;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.CltComplaintReplyService;
...
*/
@Controller
@RequestMapping("/cltComplaintReply")
public class CltComplaintReplyController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(CltComplaintReplyController.class);
	
	private static final String BASE_PATH = "cltComplaintReply/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = cltComplaintReplyService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			CltComplaintReply entity = cltComplaintReplyService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<CltComplaintReply>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<CltComplaintReply>> result = new HttpResult<Pagination<CltComplaintReply>>();
		try {
			Pagination<CltComplaintReply> data = cltComplaintReplyService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
