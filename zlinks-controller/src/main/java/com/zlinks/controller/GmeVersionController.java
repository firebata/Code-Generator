...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.GmeVersion;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.GmeVersionService;
...
*/
@Controller
@RequestMapping("/gmeVersion")
public class GmeVersionController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(GmeVersionController.class);
	
	private static final String BASE_PATH = "gmeVersion/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = gmeVersionService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			GmeVersion entity = gmeVersionService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<GmeVersion>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<GmeVersion>> result = new HttpResult<Pagination<GmeVersion>>();
		try {
			Pagination<GmeVersion> data = gmeVersionService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
