...
public class GmeVersionController extends BaseController {
	...
	/**
     * @api {post} /gmeVersions/save 01. GmeVersion新增
	...
	public JsonResult add(@RequestBody GmeVersion gmeVersion) {
  		...
  		RestDoing doing = jsonResult -> {
            ...
            int counts = gmeVersionServiceImpl.add(gmeVersion);
	...
	public JsonResult deleteJson(@PathVariable("id") int id) {
		RestDoing doing = jsonResult -> {
            ...
            int counts = gmeVersionServiceImpl.delete(gmeVersion);
            jsonResult.data = counts;
        };
        return doing.go(request, logger);
	}
	...
	public String info(@PathVariable("id") Long id) {
		...
		RestDoing doing = jsonResult -> {
			...
			GmeVersion entity  = gmeVersionServiceImpl.queryInfoById(id);
	...
	@ResponseBody
	public JsonResult<Pagination<GmeVersion>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		JsonResult<Pagination<GmeVersion>> result = new JsonResult<Pagination<GmeVersion>>();
		try {
			Pagination<GmeVersion> data = gmeVersionService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
