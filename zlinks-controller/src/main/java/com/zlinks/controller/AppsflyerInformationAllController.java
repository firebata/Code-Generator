...
*/
@Controller
@RequestMapping("/appsflyerInformationAll")
public class AppsflyerInformationAllController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(AppsflyerInformationAllController.class);
	
	private static final String BASE_PATH = "appsflyerInformationAll/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = appsflyerInformationAllService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			AppsflyerInformationAll entity = appsflyerInformationAllService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<AppsflyerInformationAll>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<AppsflyerInformationAll>> result = new HttpResult<Pagination<AppsflyerInformationAll>>();
		try {
			Pagination<AppsflyerInformationAll> data = appsflyerInformationAllService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
