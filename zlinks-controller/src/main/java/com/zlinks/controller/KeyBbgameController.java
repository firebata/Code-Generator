...
*/
@Controller
@RequestMapping("/keyBbgame")
public class KeyBbgameController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(KeyBbgameController.class);
	
	private static final String BASE_PATH = "keyBbgame/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = keyBbgameService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			KeyBbgame entity = keyBbgameService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<KeyBbgame>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<KeyBbgame>> result = new HttpResult<Pagination<KeyBbgame>>();
		try {
			Pagination<KeyBbgame> data = keyBbgameService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
