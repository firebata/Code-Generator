...
*/
@Controller
@RequestMapping("/gmeNotifyUrlCopy")
public class GmeNotifyUrlCopyController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(GmeNotifyUrlCopyController.class);
	
	private static final String BASE_PATH = "gmeNotifyUrlCopy/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = gmeNotifyUrlCopyService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			GmeNotifyUrlCopy entity = gmeNotifyUrlCopyService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<GmeNotifyUrlCopy>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<GmeNotifyUrlCopy>> result = new HttpResult<Pagination<GmeNotifyUrlCopy>>();
		try {
			Pagination<GmeNotifyUrlCopy> data = gmeNotifyUrlCopyService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
