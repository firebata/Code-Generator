...
*/
@Controller
@RequestMapping("/sdkConfig")
public class SdkConfigController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(SdkConfigController.class);
	
	private static final String BASE_PATH = "sdkConfig/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = sdkConfigService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			SdkConfig entity = sdkConfigService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<SdkConfig>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<SdkConfig>> result = new HttpResult<Pagination<SdkConfig>>();
		try {
			Pagination<SdkConfig> data = sdkConfigService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
