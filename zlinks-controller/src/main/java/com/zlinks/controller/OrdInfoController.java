...
public class OrdInfoController extends BaseController {
	 ...
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(OrdInfoController.class);
	
	private static final String BASE_PATH = "ordInfo/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = ordInfoService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			OrdInfo entity = ordInfoService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<OrdInfo>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<OrdInfo>> result = new HttpResult<Pagination<OrdInfo>>();
		try {
			Pagination<OrdInfo> data = ordInfoService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
