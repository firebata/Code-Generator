...
*/
@Controller
@RequestMapping("/logCusAppPackageReport")
public class LogCusAppPackageReportController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(LogCusAppPackageReportController.class);
	
	private static final String BASE_PATH = "logCusAppPackageReport/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = logCusAppPackageReportService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			LogCusAppPackageReport entity = logCusAppPackageReportService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<LogCusAppPackageReport>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<LogCusAppPackageReport>> result = new HttpResult<Pagination<LogCusAppPackageReport>>();
		try {
			Pagination<LogCusAppPackageReport> data = logCusAppPackageReportService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
