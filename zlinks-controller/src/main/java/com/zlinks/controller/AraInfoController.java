...
*/
@Controller
@RequestMapping("/araInfo")
public class AraInfoController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(AraInfoController.class);
	
	private static final String BASE_PATH = "araInfo/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = araInfoService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			AraInfo entity = araInfoService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<AraInfo>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<AraInfo>> result = new HttpResult<Pagination<AraInfo>>();
		try {
			Pagination<AraInfo> data = araInfoService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
