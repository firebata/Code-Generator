...
*/
@Controller
@RequestMapping("/sdkConfigSwitch")
public class SdkConfigSwitchController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(SdkConfigSwitchController.class);
	
	private static final String BASE_PATH = "sdkConfigSwitch/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = sdkConfigSwitchService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			SdkConfigSwitch entity = sdkConfigSwitchService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<SdkConfigSwitch>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<SdkConfigSwitch>> result = new HttpResult<Pagination<SdkConfigSwitch>>();
		try {
			Pagination<SdkConfigSwitch> data = sdkConfigSwitchService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
