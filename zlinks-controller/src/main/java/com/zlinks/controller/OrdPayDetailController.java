...
*/
@Controller
@RequestMapping("/ordPayDetail")
public class OrdPayDetailController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(OrdPayDetailController.class);
	
	private static final String BASE_PATH = "ordPayDetail/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = ordPayDetailService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			OrdPayDetail entity = ordPayDetailService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<OrdPayDetail>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<OrdPayDetail>> result = new HttpResult<Pagination<OrdPayDetail>>();
		try {
			Pagination<OrdPayDetail> data = ordPayDetailService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
