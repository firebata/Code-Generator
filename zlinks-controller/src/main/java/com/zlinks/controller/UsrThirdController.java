...
*/
@Controller
@RequestMapping("/usrThird")
public class UsrThirdController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(UsrThirdController.class);
	
	private static final String BASE_PATH = "usrThird/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = usrThirdService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			UsrThird entity = usrThirdService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<UsrThird>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<UsrThird>> result = new HttpResult<Pagination<UsrThird>>();
		try {
			Pagination<UsrThird> data = usrThirdService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
