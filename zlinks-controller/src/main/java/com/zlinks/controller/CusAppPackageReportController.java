...
*/
@Controller
@RequestMapping("/cusAppPackageReport")
public class CusAppPackageReportController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(CusAppPackageReportController.class);
	
	private static final String BASE_PATH = "cusAppPackageReport/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = cusAppPackageReportService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			CusAppPackageReport entity = cusAppPackageReportService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<CusAppPackageReport>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<CusAppPackageReport>> result = new HttpResult<Pagination<CusAppPackageReport>>();
		try {
			Pagination<CusAppPackageReport> data = cusAppPackageReportService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
