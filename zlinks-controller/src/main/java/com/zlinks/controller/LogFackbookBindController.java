...
*/
@Controller
@RequestMapping("/logFackbookBind")
public class LogFackbookBindController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(LogFackbookBindController.class);
	
	private static final String BASE_PATH = "logFackbookBind/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = logFackbookBindService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			LogFackbookBind entity = logFackbookBindService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<LogFackbookBind>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<LogFackbookBind>> result = new HttpResult<Pagination<LogFackbookBind>>();
		try {
			Pagination<LogFackbookBind> data = logFackbookBindService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
