...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.GmeNotifyUrl;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.GmeNotifyUrlService;
...
*/
@Controller
@RequestMapping("/gmeNotifyUrl")
public class GmeNotifyUrlController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(GmeNotifyUrlController.class);
	
	private static final String BASE_PATH = "gmeNotifyUrl/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = gmeNotifyUrlService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			GmeNotifyUrl entity = gmeNotifyUrlService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<GmeNotifyUrl>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<GmeNotifyUrl>> result = new HttpResult<Pagination<GmeNotifyUrl>>();
		try {
			Pagination<GmeNotifyUrl> data = gmeNotifyUrlService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
