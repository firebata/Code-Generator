...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.LogUserLogin;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.LogUserLoginService;
...
public class LogUserLoginController extends BaseController {
	 ...
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(LogUserLoginController.class);
	
	private static final String BASE_PATH = "logUserLogin/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = logUserLoginService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			LogUserLogin entity = logUserLoginService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<LogUserLogin>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<LogUserLogin>> result = new HttpResult<Pagination<LogUserLogin>>();
		try {
			Pagination<LogUserLogin> data = logUserLoginService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
