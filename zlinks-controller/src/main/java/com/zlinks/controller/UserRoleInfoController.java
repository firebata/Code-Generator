...
public class UserRoleInfoController extends BaseController {
	...
	private static final String BASE_PATH = "userRoleInfo/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = userRoleInfoService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			UserRoleInfo entity = userRoleInfoService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<UserRoleInfo>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<UserRoleInfo>> result = new HttpResult<Pagination<UserRoleInfo>>();
		try {
			Pagination<UserRoleInfo> data = userRoleInfoService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
