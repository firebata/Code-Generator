...
*/
@Controller
@RequestMapping("/googleBuy")
public class GoogleBuyController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(GoogleBuyController.class);
	
	private static final String BASE_PATH = "googleBuy/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = googleBuyService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			GoogleBuy entity = googleBuyService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<GoogleBuy>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<GoogleBuy>> result = new HttpResult<Pagination<GoogleBuy>>();
		try {
			Pagination<GoogleBuy> data = googleBuyService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
