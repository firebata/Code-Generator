...
*/
@Controller
@RequestMapping("/cltManagerAttention")
public class CltManagerAttentionController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(CltManagerAttentionController.class);
	
	private static final String BASE_PATH = "cltManagerAttention/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = cltManagerAttentionService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			CltManagerAttention entity = cltManagerAttentionService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<CltManagerAttention>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<CltManagerAttention>> result = new HttpResult<Pagination<CltManagerAttention>>();
		try {
			Pagination<CltManagerAttention> data = cltManagerAttentionService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
