...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.RoleAuth;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.RoleAuthService;
...
public class RoleAuthController extends BaseController {
	...
	private static final String BASE_PATH = "roleAuth/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = roleAuthService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			RoleAuth entity = roleAuthService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<RoleAuth>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<RoleAuth>> result = new HttpResult<Pagination<RoleAuth>>();
		try {
			Pagination<RoleAuth> data = roleAuthService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
