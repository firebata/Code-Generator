...
public class OrdPayController extends BaseController {
	 ...
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(OrdPayController.class);
	
	private static final String BASE_PATH = "ordPay/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = ordPayService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			OrdPay entity = ordPayService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<OrdPay>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<OrdPay>> result = new HttpResult<Pagination<OrdPay>>();
		try {
			Pagination<OrdPay> data = ordPayService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
