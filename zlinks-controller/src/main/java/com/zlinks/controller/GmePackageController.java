...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.GmePackage;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.GmePackageService;
...
*/
@Controller
@RequestMapping("/gmePackage")
public class GmePackageController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(GmePackageController.class);
	
	private static final String BASE_PATH = "gmePackage/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = gmePackageService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			GmePackage entity = gmePackageService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<GmePackage>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<GmePackage>> result = new HttpResult<Pagination<GmePackage>>();
		try {
			Pagination<GmePackage> data = gmePackageService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
