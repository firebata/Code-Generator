...
public class GmePackageController extends BaseController {
	...
	/**
     * @api {post} /gmePackages/save 01. GmePackage新增
	...
	public JsonResult add(@RequestBody GmePackage gmePackage) {
  		...
  		RestDoing doing = jsonResult -> {
            ...
            int counts = gmePackageServiceImpl.add(gmePackage);
	...
	public JsonResult deleteJson(@PathVariable("id") int id) {
		RestDoing doing = jsonResult -> {
            ...
            int counts = gmePackageServiceImpl.delete(gmePackage);
            jsonResult.data = counts;
        };
        return doing.go(request, logger);
	}
	...
	public String info(@PathVariable("id") Long id) {
		...
		RestDoing doing = jsonResult -> {
			...
			GmePackage entity  = gmePackageServiceImpl.queryInfoById(id);
	...
	@ResponseBody
	public JsonResult<Pagination<GmePackage>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		JsonResult<Pagination<GmePackage>> result = new JsonResult<Pagination<GmePackage>>();
		try {
			Pagination<GmePackage> data = gmePackageService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
