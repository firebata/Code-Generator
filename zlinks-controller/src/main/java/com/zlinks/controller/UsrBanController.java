...
*/
@Controller
@RequestMapping("/usrBan")
public class UsrBanController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(UsrBanController.class);
	
	private static final String BASE_PATH = "usrBan/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = usrBanService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			UsrBan entity = usrBanService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<UsrBan>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<UsrBan>> result = new HttpResult<Pagination<UsrBan>>();
		try {
			Pagination<UsrBan> data = usrBanService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
