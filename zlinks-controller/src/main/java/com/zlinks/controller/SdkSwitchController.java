...
*/
@Controller
@RequestMapping("/sdkSwitch")
public class SdkSwitchController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(SdkSwitchController.class);
	
	private static final String BASE_PATH = "sdkSwitch/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = sdkSwitchService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			SdkSwitch entity = sdkSwitchService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<SdkSwitch>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<SdkSwitch>> result = new HttpResult<Pagination<SdkSwitch>>();
		try {
			Pagination<SdkSwitch> data = sdkSwitchService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
