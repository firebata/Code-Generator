...
public class PrdInfoController extends BaseController {
	 ...
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(PrdInfoController.class);
	
	private static final String BASE_PATH = "prdInfo/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = prdInfoService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			PrdInfo entity = prdInfoService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<PrdInfo>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<PrdInfo>> result = new HttpResult<Pagination<PrdInfo>>();
		try {
			Pagination<PrdInfo> data = prdInfoService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
