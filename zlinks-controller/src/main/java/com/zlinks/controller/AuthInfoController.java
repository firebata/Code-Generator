...
public class AuthInfoController extends BaseController {
	...
	private static final String BASE_PATH = "authInfo/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = authInfoService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			AuthInfo entity = authInfoService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<AuthInfo>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<AuthInfo>> result = new HttpResult<Pagination<AuthInfo>>();
		try {
			Pagination<AuthInfo> data = authInfoService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
