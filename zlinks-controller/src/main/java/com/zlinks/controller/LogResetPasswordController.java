...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.LogResetPassword;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.LogResetPasswordService;
...
*/
@Controller
@RequestMapping("/logResetPassword")
public class LogResetPasswordController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(LogResetPasswordController.class);
	
	private static final String BASE_PATH = "logResetPassword/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = logResetPasswordService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			LogResetPassword entity = logResetPasswordService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<LogResetPassword>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<LogResetPassword>> result = new HttpResult<Pagination<LogResetPassword>>();
		try {
			Pagination<LogResetPassword> data = logResetPasswordService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
