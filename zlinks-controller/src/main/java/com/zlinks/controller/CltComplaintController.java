...
*/
@Controller
@RequestMapping("/cltComplaint")
public class CltComplaintController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(CltComplaintController.class);
	
	private static final String BASE_PATH = "cltComplaint/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = cltComplaintService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			CltComplaint entity = cltComplaintService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<CltComplaint>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<CltComplaint>> result = new HttpResult<Pagination<CltComplaint>>();
		try {
			Pagination<CltComplaint> data = cltComplaintService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
