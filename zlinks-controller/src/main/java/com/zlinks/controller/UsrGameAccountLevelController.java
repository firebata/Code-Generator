...
*/
@Controller
@RequestMapping("/usrGameAccountLevel")
public class UsrGameAccountLevelController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(UsrGameAccountLevelController.class);
	
	private static final String BASE_PATH = "usrGameAccountLevel/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = usrGameAccountLevelService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			UsrGameAccountLevel entity = usrGameAccountLevelService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<UsrGameAccountLevel>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<UsrGameAccountLevel>> result = new HttpResult<Pagination<UsrGameAccountLevel>>();
		try {
			Pagination<UsrGameAccountLevel> data = usrGameAccountLevelService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
