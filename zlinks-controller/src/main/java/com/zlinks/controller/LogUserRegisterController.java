...
*/
@Controller
@RequestMapping("/logUserRegister")
public class LogUserRegisterController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(LogUserRegisterController.class);
	
	private static final String BASE_PATH = "logUserRegister/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = logUserRegisterService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			LogUserRegister entity = logUserRegisterService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<LogUserRegister>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<LogUserRegister>> result = new HttpResult<Pagination<LogUserRegister>>();
		try {
			Pagination<LogUserRegister> data = logUserRegisterService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
