...
*/
@Controller
@RequestMapping("/logLimitRecord")
public class LogLimitRecordController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(LogLimitRecordController.class);
	
	private static final String BASE_PATH = "logLimitRecord/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = logLimitRecordService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			LogLimitRecord entity = logLimitRecordService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<LogLimitRecord>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<LogLimitRecord>> result = new HttpResult<Pagination<LogLimitRecord>>();
		try {
			Pagination<LogLimitRecord> data = logLimitRecordService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
