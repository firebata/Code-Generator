...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.SysAppsflyerLog2;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.SysAppsflyerLog2Service;
...
*/
@Controller
@RequestMapping("/sysAppsflyerLog2")
public class SysAppsflyerLog2Controller extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(SysAppsflyerLog2Controller.class);
	
	private static final String BASE_PATH = "sysAppsflyerLog2/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = sysAppsflyerLog2Service.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			SysAppsflyerLog2 entity = sysAppsflyerLog2Service.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<SysAppsflyerLog2>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<SysAppsflyerLog2>> result = new HttpResult<Pagination<SysAppsflyerLog2>>();
		try {
			Pagination<SysAppsflyerLog2> data = sysAppsflyerLog2Service.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
