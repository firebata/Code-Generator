...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.OrdRefund;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.OrdRefundService;
...
*/
@Controller
@RequestMapping("/ordRefund")
public class OrdRefundController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(OrdRefundController.class);
	
	private static final String BASE_PATH = "ordRefund/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = ordRefundService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			OrdRefund entity = ordRefundService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<OrdRefund>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<OrdRefund>> result = new HttpResult<Pagination<OrdRefund>>();
		try {
			Pagination<OrdRefund> data = ordRefundService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
