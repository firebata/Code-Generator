...
public class OrdRefundController extends BaseController {
	...
	/**
     * @api {post} /ordRefunds/save 01. OrdRefund新增
	...
	public JsonResult add(@RequestBody OrdRefund ordRefund) {
  		...
  		RestDoing doing = jsonResult -> {
            ...
            int counts = ordRefundServiceImpl.add(ordRefund);
	...
	public JsonResult deleteJson(@PathVariable("id") int id) {
		RestDoing doing = jsonResult -> {
            ...
            int counts = ordRefundServiceImpl.delete(ordRefund);
            jsonResult.data = counts;
        };
        return doing.go(request, logger);
	}
	...
	public String info(@PathVariable("id") Long id) {
		...
		RestDoing doing = jsonResult -> {
			...
			OrdRefund entity  = ordRefundServiceImpl.queryInfoById(id);
	...
	@ResponseBody
	public JsonResult<Pagination<OrdRefund>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		JsonResult<Pagination<OrdRefund>> result = new JsonResult<Pagination<OrdRefund>>();
		try {
			Pagination<OrdRefund> data = ordRefundService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
