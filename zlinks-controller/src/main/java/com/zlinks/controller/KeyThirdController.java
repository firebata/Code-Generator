...
*/
@Controller
@RequestMapping("/keyThird")
public class KeyThirdController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(KeyThirdController.class);
	
	private static final String BASE_PATH = "keyThird/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = keyThirdService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			KeyThird entity = keyThirdService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<KeyThird>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<KeyThird>> result = new HttpResult<Pagination<KeyThird>>();
		try {
			Pagination<KeyThird> data = keyThirdService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
