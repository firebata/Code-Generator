...
*/
@Controller
@RequestMapping("/logUsrBan")
public class LogUsrBanController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(LogUsrBanController.class);
	
	private static final String BASE_PATH = "logUsrBan/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = logUsrBanService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			LogUsrBan entity = logUsrBanService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<LogUsrBan>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<LogUsrBan>> result = new HttpResult<Pagination<LogUsrBan>>();
		try {
			Pagination<LogUsrBan> data = logUsrBanService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
