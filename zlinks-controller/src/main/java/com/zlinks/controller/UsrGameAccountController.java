...
*/
@Controller
@RequestMapping("/usrGameAccount")
public class UsrGameAccountController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(UsrGameAccountController.class);
	
	private static final String BASE_PATH = "usrGameAccount/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = usrGameAccountService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			UsrGameAccount entity = usrGameAccountService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<UsrGameAccount>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<UsrGameAccount>> result = new HttpResult<Pagination<UsrGameAccount>>();
		try {
			Pagination<UsrGameAccount> data = usrGameAccountService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
