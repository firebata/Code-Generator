...
import com.zlinks.zlinks-repository/src/main/java/com/zlinks/domain/.UserGame;
import com.zlinks.zlinks-service/src/main/java/com/zlinks/service/.UserGameService;
...
public class UserGameController extends BaseController {
	...
	private static final String BASE_PATH = "userGame/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = userGameService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			UserGame entity = userGameService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<UserGame>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<UserGame>> result = new HttpResult<Pagination<UserGame>>();
		try {
			Pagination<UserGame> data = userGameService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
