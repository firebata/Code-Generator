...
*/
@Controller
@RequestMapping("/logUserSignIn")
public class LogUserSignInController extends BaseController {

	/**
	 * logger 日志
	 */
	private static final Logger logger = LoggerFactory.getLogger(LogUserSignInController.class);
	
	private static final String BASE_PATH = "logUserSignIn/";
	...
	public HttpResult<Integer> deleteJson(@PathVariable("id") int id) {
		...
		try {
			int row = logUserSignInService.delete(id);
	...
	public String view(@PathVariable("id") int id, ModelMap modelMap) {
		try {
			LogUserSignIn entity = logUserSignInService.getById(id);
	...
	@ResponseBody
	public HttpResult<Pagination<LogUserSignIn>> jsonList(String findContent, ModelMap modelMap, Integer pageNo) {
		HttpResult<Pagination<LogUserSignIn>> result = new HttpResult<Pagination<LogUserSignIn>>();
		try {
			Pagination<LogUserSignIn> data = logUserSignInService.findPage(modelMap, pageNo, pageSize);
			result.setData(data);
		} catch (Exception e) {
